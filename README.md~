...
*load
_load_ will open a filedialog window to search for images to load.

*mask
_mask_ will compute the eigenvectors an instantly applies the mask based on the vectors. One can switch between the computed eigenvectors by recurring pressing the _mask_ button.

*invert mask
_inver mask_ since the mask will be applied by the sign of the vector entries, one can invert the mask resp. change the sign.

*supervise
_supervise_ if one labeled the image by drawing green and blue on the canvas, the supervise button will perform a convexity splitting scheme to improve the segmentation

*save
_save_ if one is satisfied with the mask it can be saved. The button will open a save file dialog
